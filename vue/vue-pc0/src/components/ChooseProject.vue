<template>
  <div class="container">
    <el-dialog :visible.sync="visible" :modal="false" :show-close="false" class="dialog" width="926px">
      <div slot="title" style="display:flex;justify-content:space-between">
        <el-cascader
          ...
          "
        />
        <el-input v-model="keyword" placeholder="搜索项目" style="margin-left:10px" @input="getList"></el-input>
      </div>
        ...
        <div class="list-wrap" v-loading="loading">
          <el-scrollbar class="custom-scrollbar">
            <div v-for="item in list" :key="item.id" class="list-item" @click="getItem(item)">
              <div class="img-wrap">
                <el-image class="img" :src="$store.state.config.convertImgUrl(item.image)" fit="contain" />
              </div>
              <div class="content">
                <div class="title" v-text="item.projectName"></div>
                <div class="area">
                  <span>行政区域:</span>
                  <el-tag type="success" size="mini" v-text="item.areaName" style="margin-left:4px"></el-tag>
                </div>
              </div>
            </div>
          </el-scrollbar>
    ...
    <el-button v-show="!name.length" type="primary" @click="handleShow">选择项目</el-button>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      getList: this.$debounce(() => {
        this.loading = true
        this.$http
          .get('/carp/business/a/q/project/area/name', {
            params: {
              name: this.keyword,
  ...
  methods: {
    ...
    // 获取信息项
    getItem(info) {
      this.$emit('sendId', info.projectId)
    ...
    getDetails(id) {
      ...
      this.clearValidate()
      this.$http.get(`/carp/business/a/q/project/${id}`).then(({ code, data, message }) => {
        if (code === 0) {
          this.name = data.name
          this.$emit('change', {
            id,
          ...
          this.$message.error('获取项目信息失败: ' + message)
        }
      })
    },
    // 清除当前组件
