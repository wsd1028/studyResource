  ...
  >
    <el-dialog :visible.sync="uploadDialogVisible" width="900px" center @close="uploadDialogNo">
      ...
      >
        ...
        <el-table-column prop="login" label="账号" align="center">
          <template slot-scope="scope">
            <span :style="{color:(scope.row.insertDescr?'red':'')}">{{scope.row.login}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="realName" label="姓名" align="center">
          <template slot-scope="scope">
            <span :style="{color:(scope.row.insertDescr?'red':'')}">{{scope.row.realName}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="email" label="email" align="center">
          <template slot-scope="scope">
            <span :style="{color:(scope.row.insertDescr?'red':'')}">{{scope.row.email}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="phoneNum" label="电话" align="center">
          <template slot-scope="scope">
            <span :style="{color:(scope.row.insertDescr?'red':'')}">{{scope.row.phoneNum}}</span>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="账号" prop="login">
          <el-input v-model="updateForm.login" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="姓名" prop="realName">
        ...
        <el-form-item label="电话" prop="phoneNum">
          <el-input v-model="updateForm.phoneNum" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <div class="top">
      <div class="top-left">
        ...
        <div class="top-link">
          <el-button type="primary" size="small" @click="uploadTem('templete/系统账号模板.xlsx')">下载模板</el-button>
          <el-upload
            class="upload-demo top-right"
            ref="upload"
            :limit="1"
            action="api/Resource/UploadNodeManager"
    ...
    >
      ...
      <el-table-column prop="login" label="账号" align="center"></el-table-column>
      <el-table-column prop="realName" label="姓名" align="center"></el-table-column>
      <el-table-column prop="email" label="email" align="center"></el-table-column>
      <el-table-column prop="phoneNum" label="电话" align="center"></el-table-column>
      <el-table-column prop align="center" label="操作" width="350px">
        <template slot-scope="scope">
          <el-button
            type="primary"
            size="mini"
...
export default class SystemAccount extends Vue {
  ...
  private PageData: Array<NodeManager> = [];
  ...
  private handleDelete(index: number, row: object) {
    this.$confirm("此操作将永久删除该管理员, 是否继续?", "提示", {
      ...
      .then(() => {
        ...
        let randomNum = Math.round(Math.random() * 10000);
        let url = `api/Resource/DelNodeManager?id=${id}&randomNum=${randomNum}`;
        this.deleteData(url);
      })
      .catch(() => {
        this.$message({
  ...
  //修改密码
  private async updatePwd(id: string, pwd: string) {
    let randomNum = Math.round(Math.random() * 10000);
    let url = `api/Resource/UpdatePwd?id=${id}&pwd=${pwd}&randomNum=${randomNum}`;
  ...
  private async handlePwd(index: number, row: object) {
    let pwdIndex = index;
    let pageData = this.PageData;
    this.$prompt("请输入密码", "提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      inputPattern: /^(?=.*[a-zA-Z])(?=.*\d)(?=.*\D)(?=.*[~!@#$%^&*()_+`\-={}:";'<>?,.\/]).{8,}$/,
      inputErrorMessage:
        "密码格式不正确(必须包含大小写字母，数字，特是字符8位以上)"
    })
      .then(({ value }) => {
        let pwd = value;
        let id = pageData[index].id;
        this.updatePwd(id, pwd);
      })
      .catch(() => {
        this.$message({
          duration: 2000,
  ...
  private updateForm: NodeManager = null;
  private handleUpdate(index: number, row: NodeManager) {
  ...
  private updateDialogYes(formName: any) {
    this.$refs[formName].validate(async valid => {
      if (valid) {
        let randomNum = Math.round(Math.random() * 10000);
        let url = `api/Resource/UpdateNodeManager?randomNum=${randomNum}`;
  ...
  //查询
  private searchValueLogin: string = "";
  ...
  private async handleSearch(pageIndex) {
    ...
    let RealName = this.searchValueName;
    let Login = this.searchValueLogin;
    let PageSize = this.pagesize;
    let randomNum = Math.round(Math.random() * 10000);
    let url = `api/Resource/GetNodeManagerList?RealName=${RealName}&Login=${Login}&PageIndex=${pageIndex}&PageSize=${PageSize}&randomNum=${randomNum}`;
    url = encodeURI(url);
    let res = await Axios.get(url);
    if (res.data.code == 1) {
