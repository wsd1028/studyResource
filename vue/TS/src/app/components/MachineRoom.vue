  ...
  >
    <el-dialog :visible.sync="uploadDialogVisible" width="942px" center @close="uploadDialogNo">
      ...
      >
        ...
        <el-table-column prop="roomCode" label="机房编码" align="center">
          <template slot-scope="scope">
            <span :style="{color:(scope.row.insertDescr?'red':'')}">{{scope.row.roomCode}}</span>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="机房编码" prop="roomCode">
          <el-input v-model="addForm.roomCode" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      >
        <!-- <el-form-item label="选择节点" prop="nodeID">
          <el-select v-model="updateForm.nodeID" placeholder="请选择">
            <el-option
              v-for="item in nodeMess"
              :key="item.nodeId"
              :label="item.nodeName"
        ...
        <el-form-item label="机房名称" prop="roomName">
          <el-input v-model="updateForm.roomName" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="机房编码" prop="roomCode">
          <el-input v-model="updateForm.roomCode" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <div class="top-left">
        <div class="top-left1">
          ...
          <div class="search">
            ...
            <div class="searchRule" style="marginRight:10px">
              <span>机房编码：</span>
              <el-input
        ...
        <div class="top-link">
          <el-button type="primary" size="small" @click="uploadTem('templete/机房模板.xls')">下载模板</el-button>
          <el-upload
            class="upload-demo top-right"
            ref="upload"
            :limit="1"
            action="api/Resource/UploadRoom"
    ...
    <el-table :data="PageData" class="marginTop" v-loading="loading" border style="width: 100%">
      ...
      <el-table-column prop="roomCode" label="机房编码" align="center"></el-table-column>
...
export default class MachineRoom extends Vue {
  //下载模板
  private async uploadTem(url: string) {
    window.location.href = url;
  }
  private tableData: Array<Room> = [];
  ...
  private addForm: Room = {};
  private fromRules: object = {
    NodeID: [{ trigger: "blur", required: true, message: "节点不能为空" }],
    roomName: [
      { trigger: "blur", required: true, message: "机房名称不能为空" }
    ],
    roomCode: [{ trigger: "blur", required: true, message: "机房编码不能为空" }]
  };
  private handleAdd() {
  ...
  private handleDelete(index: number, row: object) {
    this.$confirm("此操作将永久删除该机房信息, 是否继续?", "提示", {
      ...
      .then(async () => {
        ...
        let randomNum = Math.round(Math.random() * 10000);
        let url = `api/Resource/DelRoom?id=${id}&randomNum=${randomNum}`;
        let res = await Axios.get(url);
        if (res.data.code == 1) {
          this.$message({
            duration: 2000,
  ...
  private updateForm: Room = null;
  private handleUpdate(index: number, row: Room) {
  ...
  private updateDialogYes(formName: any) {
    this.$refs[formName].validate(async valid => {
      if (valid) {
        let randomNum = Math.round(Math.random() * 10000);
        let url = `api/Resource/UpdateRoom?randomNum=${randomNum}`;
  ...
  private searchRoomCode: string = "";
  private async handleSearch() {
    let roomName = this.searchRoomName;
    let roomCode = this.searchRoomCode;
    let randomNum = Math.round(Math.random() * 10000);
    ...
    if (res.data.code == 1) {
      ...
      searchData = tableData.filter(item => {
        return (
          item.roomName.includes(roomName) && item.roomCode.includes(roomCode)
        );
      });
