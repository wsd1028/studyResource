  ...
  <div class="demo">
    ...
    >
      ...
      <div v-if="mapConfig.zoom < 12">
        <bm-marker
          :icon="{
            url: require('@/assets/svg/city.svg'),
            size: { width: 164, height: 97 }
          }"
          :key="index"
        ...
        >
          <bm-label :content="item.name" :labelStyle="cityLabel" :offset="{ width: 0, height: 30 }" />
          <bm-label :content="item.value + ''" :labelStyle="cityLabel" :offset="{ width: 0, height: 45 }" />
        </bm-marker>
      </div>
      <bm-marker
        :icon="{
