    ...
    <section>
      <van-tabs color="#4683f1" swipeable>
        ...
        <van-tab title="督办结果" v-if="mainData.state != $dictionaries.dispatch.wait">
          ...
          <div class="btns" v-if="mainData.receiverId == $store.state.user.user.id">
            <van-button
              @click="handelFix(mainData)"
              block
              class="mybtn"
              type="info"
              v-if="mainData.state == $dictionaries.dispatch.handleing || mainData.state == $dictionaries.dispatch.reject"
            >
              <!-- eslint -->
              立即处理
            </van-button>
