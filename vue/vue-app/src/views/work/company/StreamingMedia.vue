<template>
  <div class="company-streamingMedia mainBox">
    <div class="mainTop">
          ...
          <van-list :finished="finished" @load="select" finished-text="没有更多了" v-model="loading">
            <div :key="index" class="item" v-for="(item, index) in list">
              <div class="top">
                ...
                <span :class="['state-span', item.status ? 'finishState' : 'cancelState']" @click="handelItem(item)">播放</span>
              </div>
              <div class="bottom">
                <p>
                  <span>设备状态:</span>
...
export default {
  ...
  mounted() {
    this.searchData.workId = this.$store.state.user.workId
  },
  methods: {
    //点击每一项,跳转详情界面
    handelItem(params) {
    ...
    async select(page) {
      ...
      let url = '/carp/business/a/q/business/msworklive/work/list'
      let data = {
        list: this.list,
