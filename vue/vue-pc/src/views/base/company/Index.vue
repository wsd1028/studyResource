<template>
  <div class="container">
    <common-table-control
      ...
      :filters="filters"
      :bind-buttons="[{ label: '运渣车', rule: { regExp: /145/, key: 'typeId' } }]"
      @bindButtonClick="
        company = $event.row
        bindCarShow = true
      "
    ></common-table-control>
    <el-dialog title="绑定运渣车" v-if="bindCarShow" :visible="true" top="10vh" width="600px" @close="bindCarShow = false">
      <car-bind :company="company"></car-bind>
    </el-dialog>
  </div>
</template>
<script>
import commonTableControl from '@/components/CommonTableControl'
import carBind from './CarBind'
export default {
  name: 'company',
  data() {
    // 表操作权限
  ...
  methods: {
    ...
    getCompanyType() {
      this.$http.get('/carp/business/a/q/dict/category/company_type').then(({ code, data }) => {
        ...
        this.form.label.typeId.item = items
        this.filters.typeId.item = [{ label: '全部类型', code: null }].concat(items)
      })
    }
  },
  created() {
