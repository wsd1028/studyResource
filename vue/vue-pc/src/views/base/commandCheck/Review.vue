<template>
  <el-row :gutter="10" class="details-container">
    ...
    <!-- 企业处理时上传图片 -->
    <el-col v-if="[30].includes($userType) && stateIndex == 0" style="display:flex;flex-flow:column nowrap">
      <el-card class="box-card" shadow="hover" style="flex:auto;overflow:visible">
        <div slot="header" class="clearfix title">
          <span>上传图片</span>
        </div>
    ...
    <!-- 查看指派点检图片及政府审核 -->
    <el-col v-if="[0, 50].includes($userType) || stateIndex !== 0" style="flex-flow:column nowrap">
      <el-row :gutter="10">
        <el-col v-for="(item, index) in details.list" :key="item.id" :lg="24" style="flex-flow:column nowrap">
          <el-card class="box-card" shadow="hover" style="overflow:visible">
            <div slot="header" class="clearfix" style="display:flex;align-items:center;justify-content: space-between">
              <div>
                <!-- 审核项名称 -->
                <span v-text="`${index + 1}.${item.dictName}`" style="font-weight: bold"></span>
              ...
              <div v-if="stateIndex == 1" class="review-wrap">
                <el-radio-group style="display: flex" v-model="reviewFormData[`${item.dictId}-checkState`]">
                  <el-radio :label="true" style="margin-right:20px">合格</el-radio>
                  <div class="redRadio">
            ...
            <div class="content" style="display:block">
              ...
              <div v-if="stateIndex == 1 && [0, 50].includes($userType)" class="review-wrap" style="margin-top:10px">
                <el-input type="textarea" :rows="3" v-model="reviewFormData[`${item.dictId}-remark`]" placeholder="审核备注" style="margin-top:10px"></el-input>
              </div>
            </div>
          </el-card>
        </el-col>
