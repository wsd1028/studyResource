...
export default {
  ...
  methods: {
    // 获取播放地址
    getUrl() {
      this.$http.put(`/carp/business/a/q/business/mserverchannel/status/${this.mserverchannel.channelIdentifier}`).then(({ code, data, message }) => {
        if (code === 0) {
          this.url = data.hls
        ...
        .put(
          ...
          {},
          {
            params: {
              action: flag ? 1 : 0,
              direction: code,
              id: this.mserverchannel.channelIdentifier
            }
          }
        )
        .then(({ code, message }) => {
