      ...
      <div class="left-content-wrap">
        <el-scrollbar class="custom-scrollbar">
          <el-row :gutter="20" style="margin:0">
            <el-col :md="12" :lg="8" :xl="6" v-for="(item, key) in reviewList" :key="key" class="card-wrap">
              <div class="card">
                <!-- 标题 -->
                <div class="title">
                  <span v-cloak>{{ '电子联单号: ' + item.electronicFormId }}</span>
                </div>
                <!-- 图片 -->
                <div class="pic-wrap">
                  <div class="pic" @click="handleDetails(item)">
                    <el-image fit="contain" :src="$store.state.config.convertImgUrl(item.carPhoto)" />
                  </div>
                </div>
                <!-- 按钮 -->
                <div class="wrap-bt">
                  <el-row>
                    <el-col v-if="item.headId">
                      <el-button :type="item.clean ? 'success' : 'danger'" plain v-text="item.clean ? '干净' : '不干净'"></el-button>
                      <el-button :type="item.cover ? 'success' : 'danger'" plain v-text="item.cover ? '覆盖' : '未覆盖'"></el-button>
                      <el-button :type="item.checkReviewed ? 'success' : 'danger'" plain v-text="item.checkReviewed ? '已检查' : '未检查'">未点检</el-button>
                    </el-col>
                    <el-col style="margin-top:4px;text-align:center">
                      <el-button v-if="!item.headId" type="primary" @click="handleReview(item)">审核</el-button>
                    </el-col>
                  </el-row>
                </div>
              </div>
        ...
        <el-col>
            ...
            <div class="content">
              ...
              <div style="flex:none;width:100%">
                ...
                <el-image
                  ...
                  :preview-src-list="electronicForm.data.adminPhoto.map(item => $store.state.config.convertImgUrl(item))"
                  style="height:100px;width:100px;background-color:black;margin:2px"
                  fit="contain"
                >
                  <div slot="error" class="image-slot" style="height:100px;width:100px;text-align:center;line-height:100px">
          ...
          <el-card shadow="never" class="box-card" style="overflow:visible">
            ...
            <div class="content">
              <div class="content-item">
                <el-image
                  :src="$store.state.config.convertImgUrl(electronicForm.data.driverSignImage)"
                  :preview-src-list="[$store.state.config.convertImgUrl(electronicForm.data.driverSignImage)]"
        ...
        <el-col>
            ...
            <div class="content">
              ...
              <div style="flex:none;width:100%">
                ...
                <el-image
                  ...
                  :key="key"
                  :src="$store.state.config.convertImgUrl(item)"
                  :preview-src-list="electronicForm.data.endAdminPhoto.map(item => $store.state.config.convertImgUrl(item))"
                  style="height:100px;width:100px;background-color:black;margin:2px"
                  fit="contain"
                >
                  <div slot="error" class="image-slot" style="height:100px;width:100px;text-align:center;line-height:100px">
          ...
          <el-card shadow="never" class="box-card" style="overflow:visible">
            ...
            <div class="content">
              <div class="content-item">
                <el-image
                  :src="$store.state.config.convertImgUrl(electronicForm.data.endSignImage)"
                  :preview-src-list="[$store.state.config.convertImgUrl(electronicForm.data.endSignImage)]"
                  style="height:100px;width:100px;background-color:white;margin:2px"
                  fit="contain"
                >
                  <div slot="error" class="image-slot" style="height:100px;width:100px;text-align:center;line-height:100px">
...
export default {
  ...
  methods: {
     ...
     * 获取审核分页信息
     * * */
    getReviewList(page = this.pagination.current) {
      this.$http
        .get('/carp/business/a/q/review/message/showPage', {
          params: {
        ...
        .then(({ code, data, message }) => {
          if (code === 0) {
            ...
            this.$message.error('审核分页信息请求失败:' + message)
          }
        })
    },
    /* *
