<template>
  <div class="container">
    <el-dialog :visible.sync="visible" :modal="false" :show-close="false" class="dialog" width="926px">
      <div slot="title" style="display:flex;justify-content:space-between">
        <el-cascader
          ...
          "
        />
        <el-input v-model="keyword" placeholder="搜索消纳站" style="margin-left:10px" @input="getList"></el-input>
      </div>
          ...
          <el-scrollbar class="custom-scrollbar">
            <div v-for="item in list" :key="item.id" class="list-item" @click="getItem(item)">
              ...
              <div class="content">
                ...
                <div class="type">
                  <span>类型:</span>
                  <el-tag type="success" size="mini" v-text="item.dictStr" style="margin-left:4px"></el-tag>
                </div>
              </div>
            </div>
          </el-scrollbar>
    ...
    <el-button v-show="!name.length" type="primary" @click="handleShow">选择消纳站</el-button>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      getList: this.$debounce(() => {
        this.loading = true
        this.$http
          .get('/carp/business/a/q/garbage/station/page', {
            params: {
              name: this.keyword,
  ...
  methods: {
    ...
    getDetails(id) {
      ...
      this.clearValidate()
      this.$http.get(`/carp/business/a/q/garbage/station/${id}`).then(({ code, data, message }) => {
        if (code === 0) {
          this.name = data.name
          this.$emit('change', {
            id,
          ...
          this.$message.error('获取消纳站信息失败: ' + message)
        }
      })
    },
    // 清除当前组件
