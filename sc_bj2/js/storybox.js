...
var StoryBox = {
  ...
  setContainerPanels : function() {
    ...
    this.containerObject.find("[data-animation ^= 'animated']").each(function(){
      objectInstance.containerPanels[objectInstance.containerPanels.length]
          = {
        'container'       : jQuery(this),
        'displayEffect'   :
            (jQuery.trim(jQuery(this).attr('data-animation')) == '' ?
                objectInstance.settings.displayEffect :
                (
                    jQuery(this).attr('data-animation').indexOf(',') != -1
                        ? jQuery(this).attr('data-animation').split(',')
                        : jQuery(this).attr('data-animation')
                    )
                ),
        'hideEffect'      :
            typeof jQuery(this).attr('data-animation-hide') != "undefined" ?
                (
                    jQuery.trim(jQuery(this).attr('data-animation-hide')) == '' ?
                        objectInstance.settings.hideEffect :
                        (
                            jQuery(this).attr('data-animation-hide').indexOf(',') != -1
                                ? jQuery(this).attr('data-animation-hide').split(',')
                                : jQuery(this).attr('data-animation-hide')
                            )
                    ) : false
      };
      $(this).css('opacity', 0);
